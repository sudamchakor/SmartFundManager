// src/utils/investmentCalculators.js
...
export const calculateSwp = (
  ...
  for (let i = 0; i < numberOfMonths; i++) {
    remainingPrincipal = remainingPrincipal * (1 + monthlyRate) - withdrawalPerMonth;
...
export const calculateFd = (principal, annualRate, timePeriod, compoundingFrequency) => {
  ...
  const futureValue = principal * Math.pow(1 + (annualRate / 100) / n, n * timePeriod);
  const investedAmount = principal;
  const estimatedReturns = futureValue - investedAmount;
  return { investedAmount, estimatedReturns, totalValue: futureValue };
};
